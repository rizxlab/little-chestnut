      ...
      <section className="settings-panel">
        ...
        <div className="theme-choice-grid" role="group" aria-label={tr("外观模式", "Appearance")}>
          <button className={props.theme === "light" ? "selected" : ""} type="button" aria-pressed={props.theme === "light"} onClick={() => props.setTheme("light")}>
            <span aria-hidden="true">☀</span><strong>{tr("日间", "Light")}</strong><small>{tr("温暖明亮", "Warm and bright")}</small>
          </button>
          <button className={props.theme === "dark" ? "selected" : ""} type="button" aria-pressed={props.theme === "dark"} onClick={() => props.setTheme("dark")}>
            <span aria-hidden="true">☾</span><strong>{tr("夜间", "Dark")}</strong><small>{tr("柔和低亮", "Soft and dim")}</small>
          </button>
        </div>
      </section>
        ...
        <div className="milestone-threshold-list">
          <div className="milestone-threshold-row">
            ...
            <div role="group" aria-label={tr("调整进阶效果次数", "Adjust glow threshold")}>
              <button type="button" disabled={props.cardMilestoneFirst <= 1} onClick={() => props.setCardMilestoneFirst((current) => Math.max(1, current - 1))}>−</button>
              ...
              <button type="button" disabled={props.cardMilestoneFirst >= props.cardMilestoneSecond - 1} onClick={() => props.setCardMilestoneFirst((current) => Math.min(props.cardMilestoneSecond - 1, current + 1))}>＋</button>
            </div>
          </div>
          <div className="milestone-threshold-row">
            ...
            <div role="group" aria-label={tr("调整高级效果次数", "Adjust stellar threshold")}>
              <button type="button" disabled={props.cardMilestoneSecond <= props.cardMilestoneFirst + 1} onClick={() => props.setCardMilestoneSecond((current) => Math.max(props.cardMilestoneFirst + 1, current - 1))}>−</button>
              ...
              <button type="button" disabled={props.cardMilestoneSecond >= 99} onClick={() => props.setCardMilestoneSecond((current) => Math.min(99, current + 1))}>＋</button>
            </div>
          </div>
        </div>
      </section>
