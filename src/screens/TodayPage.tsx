    ...
    <div className="screen tab-screen" data-tab="today" aria-hidden={!props.active}>
      <section className="welcome">
        <div className="welcome-meta">
          <button className="calendar-entry-button" type="button" aria-label={`${tr("打开日历", "Open calendar")}，${new Intl.DateTimeFormat(props.locale, { year: "numeric", month: "long", day: "numeric", weekday: "long" }).format(props.clockNow)}`} onClick={props.onOpenCalendar}><AppIcon name="calendar" /></button>
      ...
      <section className="content-section today-actions-section">
        <button className="temporary-action-add" type="button" onClick={props.onAddTemporaryAction}><AppIcon name="add" /><div><strong>{tr("添加临时小事", "Add a temporary action")}</strong></div><i aria-hidden="true"><AppIcon name="timer" /></i></button>
